...
- **Session-aware UI**: the login page must validate any stored token before redirecting, and public auth actions must not trigger forced redirect loops on expected `401` responses
...
| `getCurrentUser()` | Read stored username from `localStorage` |
| `setToken(token)` | Write JWT to `localStorage` |
| `setCurrentUser(username)` | Persist username for friendly UI state |
| `clearToken()` | Remove JWT from `localStorage` |
| `clearSession()` | Clear token and username together |
| `isLoggedIn()` | Returns `!!getToken()` |
| `apiFetch(path, options)` | Fetch wrapper: injects `Authorization` header, supports `authRedirect: false` for login/register, parses JSON, returns `null` on 204 |
| `restoreSession()` | Validate stored token via `/api/auth/profile`, hydrate username, clear stale sessions |
| `requireAuth()` | Redirect to `/` if not logged in or token is stale |
...
**`auth.js` behavior:**
1. On load: call `restoreSession()`, and redirect to `/dashboard.html` only when the stored token is still valid
2. Toggle between login/register sections
3. Login submit → `apiFetch('/auth/login', { authRedirect: false, ... })` → `setToken()` + `setCurrentUser()` → redirect
4. Register submit → `apiFetch('/auth/register', ...)` → show success → switch to login
5. Error display with 4-second auto-hide

**Important auth UX rule:** failed login attempts should show an inline error and stay on the login page. They must not clear the page or cause a redirect loop.
...
- Welcome panel: `#welcome-title`, `#welcome-subtitle` for a friendly landing message such as `Hey <username>, how are you doing today?`
...
1. On load: `await requireAuth()`, render the greeting, then call `loadStats()`
...
4. Logout button → `clearSession()` → redirect to `/`
...
- Small signed-in label: `#user-label`
...
1. On load: `await requireAuth()`, set `#user-label`, then `loadEmployees()`
...
10. Logout button → `clearSession()` → redirect to `/`
...
| 9 | Landing panel | Welcome card with greeting, darker gradient background |
| 10 | Table | Collapsed borders, hover row highlight, `overflow-x: auto` |
| 11 | Modal | Fixed overlay, centered white card, scrollable |
| 12 | Responsive | `@media (max-width: 768px)`: stack navbar, 2-col stats. `@media (max-width: 480px)`: 1-col stats, smaller table padding |
